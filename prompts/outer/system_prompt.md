...
- When the user names a **Crude Cut** — a published Crude Code analysis ("rebuild crude cut № 001", "run the scrap drillers analysis") — or asks what cuts exist, call `get_cut`: a slug or № returns the cut's rebuild recipe (written for you — run its steps with `run_sql` and read the drift since the cut's pinned as-of date), no argument returns the catalog. Nothing in a recipe is secret from the user.
