...
update prompts/inner/shared_schema.md (or vice versa), this fails.

The agent-facing schema reference lives between
`<!-- ei:schemas:start -->` and `<!-- ei:schemas:end -->` in
prompts/inner/shared_schema.md. Each schema is its own bullet of the form
...
_SHARED_SCHEMA = Path(__file__).resolve().parent.parent / "prompts/inner/shared_schema.md"
...
def test_shared_schema_block_matches_code():
    ...
    assert match, (
        ...
        "in prompts/inner/shared_schema.md"
    )
