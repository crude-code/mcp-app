...
from server.skills import template_base_url
...
def viewer_url(sha256: str) -> str:
    """Public URL of the published template (deal-sheet-<sha12>.jsx — see
    "deal-sheet template publish" in deploy.sh). Content-addressed, so prod
    and dev publish into the same directory with no version skew: each server
    ...
    return f"{template_base_url()}/deal-sheet-{sha256[:12]}.jsx"
