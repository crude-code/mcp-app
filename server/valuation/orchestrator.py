...
from server.valuation.routing import (
    classify_well, build_curve, WellState,
)
from server.valuation.econ import cashflow_components, compute_gross_revenue, npv, resolve_well_interest
from server.valuation.forecast import fit_curve, percentile_curves, project
    ...
    *,
    ...
    for api, fs in forecasts.items():
        ...
        gross = compute_gross_revenue(
            ...
            oil_diff=oil_diff, gas_diff=gas_diff,
        )
...
def _build_type_curve_with_stats(prod: dict, stream: str):
    """Median type curve from analog fits, plus (n_fit, n_skipped). Mirrors
    _build_cohort_curve but surfaces fit counts so the agent can judge analogs.
    ...
    for _api, d in prod.items():
        ...
        try:
            curves.append(fit_curve(np.arange(len(q), dtype=float), q, stream=stream, b_fixed=None))
        except ValueError:
            continue
    n_fit, n_skipped = len(curves), len(prod) - len(curves)
    if not curves:
        raise CohortError(f"no analog fit for stream={stream} ({len(prod)} tried)")
    return percentile_curves(curves, pct=0.5), n_fit, n_skipped
...
def forecast_wells_for_run(*, run_id: str | None, groups: list[dict], user_id: int = 0) -> dict:
    ...
    for g in groups:
        ...
        if analogs:
            ...
            oil_tc, n_fit, n_skipped = _build_type_curve_with_stats(analog_prod, "oil")
            gas_tc, _, _ = _build_type_curve_with_stats(analog_prod, "gas")
        ...
        if oil_tc is not None:
            ...
            gm["analog_meta"] = {"analog_apis": list(analogs), "n_fit": n_fit, "n_skipped": n_skipped}
