...
@pytest.mark.parametrize("call", [
    lambda: srv.deal_forecast_wells(forecasts=[_ENTRY]),
    lambda: srv.deal_valuation(run_id="r", params={}),
])
...
def test_deal_forecast_wells_tool_returns_echo(monkeypatch):
    ...
    out = json.loads(srv.deal_forecast_wells(forecasts=[_ENTRY]))
    assert out["run_id"] == "run-9"
    assert out["by_status"]["PDP"] == 1


def test_deal_forecast_wells_tool_surfaces_validation_failure(monkeypatch):
...
def test_deal_valuation_tool_returns_artifact_payload(monkeypatch):
    ...
    out = json.loads(srv.deal_valuation(run_id="run-9", params={
