...
- `shapes` — PLSS geometry (townships, sections) and LandtracUnit polygons (landtrac_units, landtrac_unit_wells). **Exploration only** — `shapes.*` cannot appear in a widget query, only in `run_sql` during composition.
<!-- ei:schemas:end -->
